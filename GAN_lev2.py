...
class GAN(object):
    ...
    def train(self, X_train, y_train, scaler_X, scaler_y, epochs=100000, batch=32, save_interval=5000):
        ...
        for cnt in range(epochs):
            ...
            input_gen = X_train[random_index: random_index + batch].reshape(batch, 729, )
            output_gen = y_train[random_index: random_index + batch].reshape(batch, self.width, self.height,
                                                                                    self.channels)
            g_loss1 =self.G.train_on_batch(input_gen, output_gen)

            ## train generator (stacked)
